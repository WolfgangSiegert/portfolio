...
<main id="inhalt">
  ...
  <section class="section" id="projekte" aria-labelledby="projects-title">
    ...
    <?php foreach ($profile['projects'] as $project): ?><article class="project <?= e($project['tone']) ?>"><div class="project-cover"><span><?= e($project['n']) ?> / <?= e($project['kind']) ?></span><h3><?= e($project['name']) ?></h3></div><div class="project-copy"><p class="meta"><?= e($project['company']) ?></p><p><?= e($project['text']) ?></p><?php tags($project['tags']); ?><?php if (isset($project['url'], $project['linkLabel'])): ?><a class="project-link" href="<?= e($project['url']) ?>"<?= !empty($project['newTab']) ? ' target="_blank" rel="noopener noreferrer"' : '' ?>><?= e($project['linkLabel']) ?> →<?php if (!empty($project['newTab'])): ?><span class="sr-only"> (öffnet einen neuen Tab)</span><?php endif; ?></a><?php endif; ?></div></article><?php endforeach; ?>
  ...
  <section class="section contact" id="kontakt" aria-labelledby="contact-title">
    <p class="section-label">08 / Kontakt</p><div><h2 id="contact-title"><?= e($profile['contact']['title']) ?></h2><p class="copy"><?= e($profile['contact']['text']) ?></p><a class="email" href="mailto:<?= e($identity['email']) ?>"><?= e($identity['email']) ?> ↗</a><div class="actions"><a href="<?= e($identity['githubUrl']) ?>" target="_blank" rel="noopener noreferrer">GitHub ↗<span class="sr-only"> (öffnet einen neuen Tab)</span></a><a href="#start">Zurück zum Anfang ↑</a></div><div class="document-links" aria-label="Dokumente zum Download"><?php foreach ($profile['documents'] as $document): ?><a href="<?= e($document['href']) ?>" download><span><strong><?= e($document['label']) ?></strong><small><?= e($document['detail']) ?></small></span><span aria-hidden="true">↓</span></a><?php endforeach; ?><a class="document-request" href="<?= e($profile['documentRequest']['href']) ?>"><span><strong><?= e($profile['documentRequest']['label']) ?></strong><small>Vollständige Fassung per E-Mail</small></span><span aria-hidden="true">↗</span></a></div></div>
  </section>
</main>
